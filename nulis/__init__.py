...
class _tulis:
    ...
    def parser(self, text):
        img, font, kata, tempkata=Image.open(myasset("temp2.jpg")), ImageFont.truetype(myasset("cekerayam4.ttf"),20),'',''
        draw=ImageDraw.Draw(img)
        if type(text) is not list:
            for i in text:
                if draw.textsize(tempkata, font)[0] < 670:
                    tempkata+=i
                else:
    ...
    def _write(self, spliter):
        img, font=Image.open(myasset("temp2.jpg")), ImageFont.truetype(myasset("cekerayam4.ttf"),20)
        draw=ImageDraw.Draw(img)
        line=130
        for i in spliter[:48]: # jumlah baris perhalaman
            draw.text((91, int(line)), i, font=font, fill=(0, 0, 0)) #selisih = Line
            line+=19+ 2.2
        return img
    ...
    def sheets(self, l:list):
        if l:
            return [l[:48]]+self.sheets(l[48:]) #jumlah baris perhalaman
        else:
            return []
    def nulis(self):
        pass
    """@property
    def tulis(self):
        img, font, kata, tempkata=Image.open(myasset("before.jpg")), ImageFont.truetype(myasset("cekerayam4.ttf"),24),'',''
        draw=ImageDraw.Draw(img)
